...
grafo_culturales  <- recuento_bigramas  %>% filter(n >= 2) %>% graph_from_data_frame()
...
grafo_institucionales  <- recuento_bigramas1  %>% filter(n >= 2) %>% graph_from_data_frame()
...
grafo_estado  <- recuento_bigramas2 %>% filter(n >= 2) %>% graph_from_data_frame()
...
grafo_mercado  <- recuento_bigramas3 %>% filter(n >=2)  %>% graph_from_data_frame()
...
obstaculizadores <- grid.arrange(plot_culturales,plot_institucionales, ncol = 2,
                                 top = textGrob("Redes de palabras",gp=gpar(fontface = "bold", fontsize = 15)))
